...
def gconnect():
    ...
    if stored_access_token is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps('Current user is already connected.'),
                                 200)
        response = make_response(json.dumps(login_session.get('username')),
                                 200)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    user_id = getUserID(email=data['email'])
    if not user_id:
    ...
    output = ''
    output += '<h1>Welcome, ' + str(login_session['user_id'])
...
def newRestaurant():
    ...
    if request.method == 'POST':
        ...
        newRestaurant = Restaurant(user_id=user_id,
